...
<body>
    ...
    <script>
        ...
        function attachCourtStationListener() {
            const element = document.getElementById('court_station');
            if (!element) return;
            const eventType = element.tagName === 'INPUT' ? 'input' : 'change';
            if (element.dataset.listenerAttached === eventType) return;
            element.addEventListener(eventType, validateCurrentStep);
            element.dataset.listenerAttached = eventType;
        }
