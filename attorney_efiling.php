...
<body>
                        ...
                        <tbody id="historyTableBody">
                            ...
                            <?php else: ?>
                                ...
                                <tr class="history-row" data-id="<?= $h['id'] ?>">
                                    <td class="date-time" data-label="Date &amp; Time"><?= date('M j, Y g:i A', strtotime($h['created_at'])) ?></td>
                                    <td data-label="File Name (Used)"><span class="file-name-cell" title="<?= htmlspecialchars($h['file_name']) ?>"><?= strlen($h['file_name']) > 20 ? substr(htmlspecialchars($h['file_name']), 0, 20) . '...' : htmlspecialchars($h['file_name']) ?></span></td>
                                    <td data-label="Category"><span class="category-badge"><?= htmlspecialchars($h['document_category'] ?? 'N/A') ?></span></td>
                                    <td data-label="Original File Name"><span class="original-file-name" title="<?= htmlspecialchars($h['original_file_name'] ?? '-') ?>"><?= strlen($h['original_file_name'] ?? '-') > 25 ? substr(htmlspecialchars($h['original_file_name'] ?? '-'), 0, 25) . '...' : htmlspecialchars($h['original_file_name'] ?? '-') ?></span></td>
                                    <td data-label="Receiver Email"><span class="receiver-email" title="<?= htmlspecialchars($h['receiver_email']) ?>"><?= strlen($h['receiver_email']) > 25 ? substr(htmlspecialchars($h['receiver_email']), 0, 25) . '...' : htmlspecialchars($h['receiver_email']) ?></span></td>
                                    <td data-label="Status"><span class="status-badge status-<?= strtolower($h['status']) ?>"><?= htmlspecialchars($h['status']) ?></span></td>
                                    <td class="history-actions" data-label="Actions">
    ...
    <style>
        ...
        .mobile-nav-toggle {
            ...
            color: #fff;
            cursor: pointer;
            box-shadow: 0 8px 18px rgba(124, 15, 47, 0.25);
            transition: transform 0.2s ease, box-shadow 0.2s ease;
            position: absolute;
            top: 20px;
            left: 20px;
        ...
        .mobile-nav-toggle:focus-visible {
            outline: 3px solid rgba(139, 21, 56, 0.45);
            outline-offset: 2px;
        }
        .mobile-nav-toggle:hover {
            transform: translateY(-1px);
            box-shadow: 0 10px 24px rgba(124, 15, 47, 0.35);
        }
        .pagination-bottom {
            margin-top: 1rem;
            padding: 0.75rem 0;
        ...
        @media (max-width: 640px) {
            ...
            .history-table tbody tr td[data-label] {
                display: flex;
                justify-content: space-between;
                align-items: center;
                gap: 12px;
            }
            .history-table tbody tr td[data-label]::before {
            ...
            .history-table tbody tr td[data-label] > span,
            .history-table tbody tr td[data-label] > .status-badge {
                text-align: right;
            }
            .history-table tbody tr td.history-actions {
                display: flex;
                flex-wrap: wrap;
            ...
            .history-table tbody tr td.history-actions::before {
                align-self: flex-start;
            }
            .history-table tbody tr td.history-actions span {
                margin-top: 4px;
            }
            .btn-view-file, 
            .btn-download-file, 
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function setSidebarState(open) {
                ...
                if (sidebarElement) {
                    sidebarElement.classList.toggle('is-visible', effectiveOpen);
